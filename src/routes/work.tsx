...
export const Route = createFileRoute("/work")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Detailed consulting case studies across business systems, operational software, corporate publishing, and security engineering." },
      { property: "og:title", content: "Engagements — Jeremiah Jideofor" },
      { property: "og:description", content: "Detailed consulting case studies across business systems, operational software, corporate publishing, and security engineering." },
  ...
  return (
    <>
      <section className="pt-36 md:pt-48 pb-16 container-page relative scanline">
        ...
        <p className="mt-10 max-w-2xl text-muted-foreground text-lg font-light leading-relaxed">
          Detailed project journals documenting scope, technical strategy, security execution, and business impact. Scoped as professional consulting engagements.
        </p>
      ...
      <section className="container-page py-16 md:py-24 relative">
        <div className="absolute inset-0 cyber-dots opacity-[0.2] pointer-events-none" />

        {projects
          .filter((p) => p.n === activeProject)
          .map((p) => (
            <Reveal key={p.n}>
              <article className="grid lg:grid-cols-12 gap-12 lg:gap-20">
                {/* Side Specifications Panel */}
                <div className="lg:col-span-4 space-y-8">
                  <div className="border border-hairline bg-surface/30 p-8 rounded relative overflow-hidden">
                    <span className="font-mono text-xs text-primary">[ ENGAGEMENT SPECS ]</span>
                    <h2 className="text-3xl font-display font-semibold uppercase tracking-tight mt-4">{p.name}</h2>
                    <p className="text-sm font-mono text-muted-foreground/80 mt-1">{p.title}</p>
                    
                    <div className="mt-8 space-y-4 font-mono text-xs text-muted-foreground border-t border-hairline/50 pt-6">
                      <div className="flex justify-between">
                        <span>SECTOR:</span>
                        <span className="text-foreground">{p.sector}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>YEAR SHIPPED:</span>
                  ...
                  {/* Tech Stack List */}
                  <div className="border border-hairline bg-surface/10 p-6 rounded">
                    <p className="eyebrow mb-4">Integrated Stack</p>
                    <div className="flex flex-wrap gap-2">
                      {p.stack.map((s) => (
                        <span key={s} className="font-mono text-[0.62rem] uppercase tracking-widest bg-background border border-hairline rounded px-2.5 py-1 text-muted-foreground">
                          {s}
                        </span>
                      ))}
                    </div>
                ...
                <div className="lg:col-span-8 space-y-12">
                  <div className="border-b border-hairline pb-6 bg-primary/5 p-6 rounded border-l-2 border-l-primary">
                    <p className="eyebrow text-primary mb-2">01 / ENGAGEMENT OVERVIEW</p>
                    <p className="text-base md:text-lg text-foreground font-light leading-relaxed">{p.overview}</p>
                  </div>

                  <div className="grid sm:grid-cols-2 gap-10">
                    <div>
                      <p className="eyebrow border-b border-hairline pb-2 mb-3">02 / THE CHALLENGE</p>
                      <p className="text-sm text-muted-foreground leading-relaxed font-light">{p.challenge}</p>
                    </div>
                    <div>
                      <p className="eyebrow border-b border-hairline pb-2 mb-3">03 / WORKFLOW RESEARCH</p>
                      <p className="text-sm text-muted-foreground leading-relaxed font-light">{p.research}</p>
                    </div>
                  </div>

                  <div className="grid sm:grid-cols-2 gap-10">
                    <div>
                      <p className="eyebrow border-b border-hairline pb-2 mb-3">04 / STRATEGIC PLAN</p>
                      <p className="text-sm text-muted-foreground leading-relaxed font-light">{p.strategy}</p>
                    </div>
                    <div>
                      <p className="eyebrow border-b border-hairline pb-2 mb-3">05 / USER EXPERIENCE DESIGN</p>
                      <p className="text-sm text-muted-foreground leading-relaxed font-light">{p.design}</p>
                    </div>
                  </div>

                  <div className="grid sm:grid-cols-2 gap-10">
                    <div>
                      <p className="eyebrow border-b border-hairline pb-2 mb-3">06 / CORE DEVELOPMENT</p>
                      <p className="text-sm text-muted-foreground leading-relaxed font-light">{p.development}</p>
                    </div>
                    <div className="border border-red-500/20 bg-red-500/5 p-4 rounded">
                      <p className="eyebrow text-red-500/90 border-b border-red-500/10 pb-2 mb-3">07 / SECURITY CONSIDERATIONS</p>
                      <p className="text-sm text-foreground/90 leading-relaxed font-light">{p.security}</p>
                    </div>
                  </div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-10 border-t border-hairline pt-10">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground leading-relaxed font-light">{p.lessons}</p>
                    </div>
                  </div>
                </div>
              </article>
