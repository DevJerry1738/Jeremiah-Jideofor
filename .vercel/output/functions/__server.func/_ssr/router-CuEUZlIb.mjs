...
useGSAP.register(gsapWithCSS);
const isPrefersReducedMotion = () => typeof window !== "undefined" && window.matchMedia("(prefers-reduced-motion: reduce)").matches;
const prefersReducedMotion = isPrefersReducedMotion();
function useGsap(callback, dependencies = [], scope) {
  return useGSAP(callback, { scope, dependencies });
}
const links = [
...
function SiteFooter() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("footer", { ref: footerRef, className: "border-t border-hairline mt-32 bg-surface/25 relative overflow-hidden", children: [
